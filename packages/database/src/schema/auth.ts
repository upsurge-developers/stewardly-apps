import { pgTable, unique, pgEnum, serial, text, integer, pgSchema, varchar, uuid, timestamp, uniqueIndex, index, jsonb, boolean, smallint, json, foreignKey, bigserial, inet } from "drizzle-orm/pg-core"
  import { sql } from "drizzle-orm"

export const auth = pgSchema("auth");
export const aalLevel = auth.enum("aal_level", ['aal1', 'aal2', 'aal3'])
export const codeChallengeMethod = auth.enum("code_challenge_method", ['s256', 'plain'])
export const factorStatus = auth.enum("factor_status", ['unverified', 'verified'])
export const factorType = auth.enum("factor_type", ['totp', 'webauthn', 'phone'])
export const oneTimeTokenType = auth.enum("one_time_token_type", ['confirmation_token', 'reauthentication_token', 'recovery_token', 'email_change_token_new', 'email_change_token_current', 'phone_change_token'])
...
(table) => {
	return {
		confirmationTokenIdx: uniqueIndex("confirmation_token_idx").using("btree", table.confirmationToken).where(sql`((confirmation_token)::text !~ '^[0-9 ]*$'::text)`),
		emailChangeTokenCurrentIdx: uniqueIndex("email_change_token_current_idx").using("btree", table.emailChangeTokenCurrent).where(sql`((email_change_token_current)::text !~ '^[0-9 ]*$'::text)`),
		emailChangeTokenNewIdx: uniqueIndex("email_change_token_new_idx").using("btree", table.emailChangeTokenNew).where(sql`((email_change_token_new)::text !~ '^[0-9 ]*$'::text)`),
		reauthenticationTokenIdx: uniqueIndex("reauthentication_token_idx").using("btree", table.reauthenticationToken).where(sql`((reauthentication_token)::text !~ '^[0-9 ]*$'::text)`),
		recoveryTokenIdx: uniqueIndex("recovery_token_idx").using("btree", table.recoveryToken).where(sql`((recovery_token)::text !~ '^[0-9 ]*$'::text)`),
		emailPartialKey: uniqueIndex("users_email_partial_key").using("btree", table.email).where(sql`(is_sso_user = false)`),
...
(table) => {
	return {
		auditLogsInstanceIdIdx: index("audit_logs_instance_id_idx").using("btree", table.instanceId),
	}
});
...
(table) => {
	return {
		instanceIdIdx: index("refresh_tokens_instance_id_idx").using("btree", table.instanceId),
		instanceIdUserIdIdx: index("refresh_tokens_instance_id_user_id_idx").using("btree", table.instanceId, table.userId),
		parentIdx: index("refresh_tokens_parent_idx").using("btree", table.parent),
		sessionIdRevokedIdx: index("refresh_tokens_session_id_revoked_idx").using("btree", table.sessionId, table.revoked),
		updatedAtIdx: index("refresh_tokens_updated_at_idx").using("btree", table.updatedAt),
...
(table) => {
	return {
		factorIdCreatedAtIdx: index("factor_id_created_at_idx").using("btree", table.userId, table.createdAt),
		userFriendlyNameUnique: uniqueIndex("mfa_factors_user_friendly_name_unique").using("btree", table.friendlyName, table.userId).where(sql`(TRIM(BOTH FROM friendly_name) <> ''::text)`),
		userIdIdx: index("mfa_factors_user_id_idx").using("btree", table.userId),
		uniqueVerifiedPhoneFactor: uniqueIndex("unique_verified_phone_factor").using("btree", table.userId, table.phone),
...
(table) => {
	return {
		mfaChallengeCreatedAtIdx: index("mfa_challenge_created_at_idx").using("btree", table.createdAt),
	}
});
...
(table) => {
	return {
		notAfterIdx: index("sessions_not_after_idx").using("btree", table.notAfter),
		userIdIdx: index("sessions_user_id_idx").using("btree", table.userId),
		userIdCreatedAtIdx: index("user_id_created_at_idx").using("btree", table.userId, table.createdAt),
	}
});
...
(table) => {
	return {
		relatesToHashIdx: index("one_time_tokens_relates_to_hash_idx").using("hash", table.relatesTo),
		tokenHashHashIdx: index("one_time_tokens_token_hash_hash_idx").using("hash", table.tokenHash),
		userIdTokenTypeKey: uniqueIndex("one_time_tokens_user_id_token_type_key").using("btree", table.userId, table.tokenType),
	}
});
...
(table) => {
	return {
		createdAtIdx: index("saml_relay_states_created_at_idx").using("btree", table.createdAt),
		forEmailIdx: index("saml_relay_states_for_email_idx").using("btree", table.forEmail),
		ssoProviderIdIdx: index("saml_relay_states_sso_provider_id_idx").using("btree", table.ssoProviderId),
	}
});
...
(table) => {
	return {
		...
		ssoProviderIdIdx: index("sso_domains_sso_provider_id_idx").using("btree", table.ssoProviderId),
	}
});
...
(table) => {
	return {
		ssoProviderIdIdx: index("saml_providers_sso_provider_id_idx").using("btree", table.ssoProviderId),
...
(table) => {
	return {
		createdAtIdx: index("flow_state_created_at_idx").using("btree", table.createdAt),
		idxAuthCode: index("idx_auth_code").using("btree", table.authCode),
		idxUserIdAuthMethod: index("idx_user_id_auth_method").using("btree", table.userId, table.authenticationMethod),
	}
});
